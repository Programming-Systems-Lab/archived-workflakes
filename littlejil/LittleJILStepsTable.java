 ...
 * two-way between tasks and the original LittleJIL steps. This is necessary:
 *   - to lookup HandlerBindings for a task: put(Task, Step) and get(Task) are used
 *   - to lookup task implementations of handler step: put(Step, Task) and get(Step) are used
 *
 * @author matias
 */
